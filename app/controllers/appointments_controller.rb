class AppointmentsController < ApplicationController
  ...
  def update
    @appointment = Appointment.where(nil)
    search_params(params).each do |key, value|
      @appointment = @appointment.update.public_send(key, value) if value.present?
    end 
    render json: @appointment, status: 200
  end
    # if @appointment.convert_start_time > Time.now #cannot update an appointment in the past
  ...
  private
    ...
    def set_appointment
      # @appointments = Appointment.find(appointment_params)
